#include <wiringPi.h>	// usar los gpio, micros, millis
...
int main(void){
    ...
    int state_led= 0;
    int state_button=0;
	int state_button_prev = 0;
	int intermitencia = 0;
	unsigned long last_time_check = millis();
	unsigned long time_check = 0;
	// Micros(); max 4294.967295 71.58s
	// cuidado del desbordamiento, comparar con maximo vlaor UINT32_MAX
	// Millis(); max 4294967295us 49.71 dias
	
    
    for (;;){
		// boton en pullup fisico
		usleep(200);		// delay antirebote 200us
		state_button = digitalRead(BUTTON);
		if (state_button_prev && !(state_button)){
			intermitencia = !(intermitencia);
		}
		state_button_prev = state_button;
		if (intermitencia){
			time_check = millis();
			if (time_check - last_time_check > 500){
