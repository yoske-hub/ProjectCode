...
////////	Cambios:
// Confirmacion de lectura de display. (procedimiento())
// Reorganizacion de la secuencia de funcionamiento y buffer.
// 
...
void procedimiento(int* d_selector){  
	...
	int selector_temp = {d_selector[0],d_selector[1],d_selector[2]};
	tempo[3]=micros();
	...
	int* estados = lectura7Segmentos(7);
		...
		int* ack_selector = lectura_selector();
		if (d_selector[0] == ack_selector[0] &&
			d_selector[1] == ack_selector[1] &&
			d_selector[2] == ack_selector[2]){
