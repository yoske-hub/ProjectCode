...
int ciclo = 4;	//	 establece el inico de orden de lectura
int ciclo_next=0;
int finish = 0;
...
const int it_a0a1a2 = 8;		//	iteraciones para confirmar lectura correcta de A0A1A2
const int it_7seg = 8;			//  iteraciones para confirmar lectura correcta de 7 segmentos
...
void procedimiento(){
	...
	value_display[ciclo-1]=caracter;
	//printf("%c",caracter);
	if (ciclo == 3){
...
void procedimiento_channel(){	
	...
	if (diff[0]>asentamiento*1.6 || diff[1]>300){
		n_error++;
		// no reinicio en ciclo 0 ya que en scan tengo mucho tiempo de lectura
		//
		ciclo = 0;
		ciclo_next = 4;
		return;
	...
	if ( (estados[0] + estados[1] + estados[2] + estados[3]) != 3 ){
		n_error++;
		//printf(" %u%u%u%u ",estados[0],estados[1],estados[2],estados[3]);
		ciclo = 0 ;
		ciclo_next = 4;
		//
		return;
		...
		for (int i = 0; i < 4; i++){
			if (estados[i] == 0){
				channel = i+1;
				if(check_channel[i]==0){
					check_channel[i]=1;	// canal check
					break;
				}else{
					ciclo=0;
					ciclo_next = 4;
	...
	printf("\t%u",channel);				
	    ...
	    ciclo = 0;
		ciclo_next = 1;
	if ((check_channel[0] + check_channel[1] + check_channel[2]) == 3){
		//printf("\n");
		finish = 1;
	}
}
...
int main(void){
	...
	/* ordenado por ciclos naturales del microcontrolador */
		switch (ciclo){
			case 1:
				/* display 1 */							
				if (data_vector[0]==0 &&
