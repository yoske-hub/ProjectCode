...
// Codigo revisado completamente de la version v1 y modificado
////////	Cambios:
// Confirmacion de lectura de canal. (procedimiento_channel())
// Almacenamiento de buffer en variables.	(procedimiento() , procedimiento_channel())
// Reorganizacion de la secuencia de funcionamiento.
// 
...
int n_channel_error = 0;		// falla o desfase en lectura de canales
int channel = 0;
...
void procedimiento(){  
	...
	int caracter = decodificarCaracter(estados);
		...
		if (debug==1){		
			...
			//printf("\t%lu::%lu::%lu",diff[0],diff[1],diff[2]);			
		}
	...
	if (etapa == 3){
		...
		//printf("\t%c%c%c",value_display[0],value_display[1],value_display[2]);
		if (tiempo)	{
			time_t current_time;
			time(&current_time);
		...
		//printf("\tErrores: %u",n_error);
		n_error = 0;	
		//fflush(stdout);	// imprimo datos alamcenados en buffer
			snprintf(buffer_display + strlen(buffer_display), sizeof(buffer_display) - strlen(buffer_display), "\n");
		//printf("\n");
...
void procedimiento_channel(){	
	...
	//printf("\t%u",channel);				
		...
		//printf("\tErrores: %u",n_error);
		n_error = 0;
		//	snprintf(buffer_canal + strlen(buffer_canal), sizeof(buffer_canal) - strlen(buffer_canal), "\tDesfases canal: %u",n_channel_error);
		//printf("\tDesfases canal: %u",n_channel_error);
		n_channel_error = 0;
		//fflush(stdout);	// imprimo datos alamcenados en buffer
		strcat(buffer_canal, buffer_display);
		printf("%s", buffer_canal);
...
int main(void){
    ...
    for (;;){
	delayMicro(10);
	int* data_vector = lectura_selector();
